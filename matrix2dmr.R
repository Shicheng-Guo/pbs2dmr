...
ttestFunction<-function(data,x1,x2){
  ...
  colnames(out)=c("Pvalue","PFDR","Delta","RA","OA")
  return(out)
}

args = commandArgs(trailingOnly=TRUE)
temp<-read.table(args[1],head=T,row.names = 1)
rlt<-ttestFunction(temp,1:9,10:19)
write.table(rlt,file=paste(args[1],".pvalue.txt",sep=""),col.names = NA,row.names = T,sep="\t",quote=F)
save(rlt,file=paste(file[i],".pvalue.RData",sep=""))
